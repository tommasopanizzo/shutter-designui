...
`@chumy/shutter` and all presets default to **visual-only** (no motor buttons or speed UI). Drag to adjust level. Pass `showControls` for the full motor panel.
...
export default function Demo() {
  return <Shutter variant="classic" size="md" />;
}
```

Notes:

- Motor UI is off by default; enable with `showControls` (and optionally `showSpeedControl`)
